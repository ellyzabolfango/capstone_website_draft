...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if ($result->num_rows === 1) {
    ...
    if ((int)$user['is_active'] !== 1) {
      $login_error = "Account is inactive.";
    } elseif (password_verify($passwordInput, $user['password'])) {
      // ✅ success
      session_regenerate_id(true); // security against fixation
      $_SESSION['user_id'] = $user['id'];
      $_SESSION['username'] = $user['username'];
      $_SESSION['role'] = $user['role']; // use DB role (admin/user)
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .input {
      ...
      background-color: #d8cdd3; color: #3e322e;
      font-size: 1rem;
    }
    .input::placeholder { color: #6b6b6b; }
    .sign-in-button {
      background-color: #0a0a59; color: white;
      border: none; padding: 14px; width: 100%;
      border-radius: 30px; font-size: 1rem;
      margin-top: 15px; cursor: pointer;
      transition: background-color 0.3s;
    }
    .sign-in-button:hover { background-color: #00003f; }
    .error-message {
      color: white; background-color: #b0423c;
