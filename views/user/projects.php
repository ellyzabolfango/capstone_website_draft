...
// ---------- create-permission: require complete annual budget docs (optional helper) ----------
$canCreate = function_exists('has_complete_annual_budget')
  ? (bool)has_complete_annual_budget($userId)
  : true; // fallback if helper isn’t defined

// ---------- HANDLE CREATE (modal Save) ----------
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'create') {
  if (function_exists('csrf_check') && !csrf_check($_POST['csrf_token'] ?? null)) {
    header('barangay: projects.php?msg=' . urlencode('Invalid request.')); exit();
  }
  if (!$canCreate) {
    header('barangay: projects.php?msg=' . urlencode('Please complete your Annual Budget docs first.')); exit();
  }
  ...
  if ($errors) {
    header('barangay: projects.php?msg=' . urlencode(implode(' • ', $errors))); exit();
  }
  ...
  // s=title, s=desc, s=file, s=impl_date, d=budget, s=category, i=created_by
  $stmt->bind_param('ssssdsi', $title, $description, $savedRel, $implDate, $budget, $TYPE, $userId);
  $ok = $stmt->execute();

  header('barangay: projects.php?msg=' . urlencode($ok ? 'Saved!' : 'Save failed'));
  exit();
}

// ---------- Fund remaining (safe if settings missing) ----------
$fundRemaining = 0.0;
...
// ---------- list Project proposals (this user's uploads) ----------
$params = [$TYPE, $userId];
$types  = 'si';
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .fund-badge { display:inline-flex; align-items:center; gap:10px; padding:8px 14px; border-radius:999px; background: rgba(0,0,0,0.25); border:1px solid rgba(255,255,255,0.35); font-weight:700; letter-spacing:.3px; }
    ...
    .btn-upload { background:#e9edf1; color:#111; font-weight:700; font-size:12px; border:none; padding:6px 14px; border-radius:999px; cursor:pointer; box-shadow: inset 0 -1px 0 rgba(0,0,0,.15); }
    .btn-upload:active { transform: translateY(1px); }
    .btn-upload[disabled] { opacity:.55; cursor:not-allowed; }

    .hint { margin-top:-6px; margin-bottom:10px; font-size:13px; opacity:.95; }
    .hint a { color:#fff; font-weight:bold; text-decoration: underline; }

    .proj-item { background:#e5e5e5; color:#000; border-radius:10px; padding:12px 14px; margin:12px 0; display:flex; justify-content:space-between; align-items:center; }
    .proj-left .t { font-weight:bold; }
    .proj-left .m { font-size:13px; color:#333; margin-top:2px; }
    .proj-right { text-align:right; }
    .proj-right .b { font-weight:bold; }
    .proj-right .d { font-size:12px; color:#333; }
    @media (max-width:640px){ .proj-item{flex-direction:column; align-items:flex-start; gap:6px;} .proj-right{text-align:left;} }
    ...
    .file-box { width: 120px; height: 84px; background:#e5e6ed; color:#000; border:1px solid #b9bfd3; border-radius:6px; display:flex; align-items:center; justify-content:center; flex-direction:column; gap:6px; }
    .file-box input { display:none; }

    .modal-done { position:absolute; right:18px; bottom:16px; background:#e9edf1; color:#111; font-weight:800; border:none; padding:8px 18px; border-radius:999px; cursor:pointer; }
  </style>
</head>
<body>
  <div class="sidebar">
    ...
    <div class="content-box">
      ...
      <div class="toolbar">
        ...
        <button
          ...
          class="btn-upload"
          id="openUpload"
          <?= $canCreate ? '' : 'disabled title="Complete Annual Budget documents first (ABYIP, CBYDP, Annual Budget, PPMP, APP, Punong Barangay Certificate, Income Statement)."' ?>
        >UPLOAD</button>
      </div>

      <?php if (!$canCreate): ?>
        <div class="hint">
          Complete your <a href="<?= htmlspecialchars(USER_URL, ENT_QUOTES, 'UTF-8') ?>/annualplan.php">Annual Budget documents</a> before creating a Project Proposal.
        </div>
      <?php endif; ?>

      <?php if (!$rows): ?>
        <div style="opacity:.9; margin-top:14px;">
          No <?= strtolower($TYPE) ?>s found for <?= htmlspecialchars($userLoc ?: 'your barangay', ENT_QUOTES, 'UTF-8') ?>.
        </div>
      <?php else: ?>
  ...
  <div class="modal" id="uploadModal" role="dialog" aria-modal="true" aria-labelledby="mTitle">
    <div class="modal-inner">
      <div class="modal-title">
        <h2 id="mTitle">ANNUAL PLAN FOR PROJECT</h2>
        <div class="line"></div>
      </div>
      ...
      <form id="createForm" method="post" enctype="multipart/form-data" class="form-grid">
        ...
        <div class="row">
          <label>TITLE:</label>
          <input type="text" name="title" placeholder="e.g., Community Fitness Equipment" required />
        </div>
      ...
      <button class="modal-done" id="doneUpload" <?= $canCreate ? '' : 'disabled' ?>>SAVE</button>
    </div>
  </div>
  ...
  <script>
    ...
    if (openBtn) openBtn.addEventListener('click', () => { if (!openBtn.hasAttribute('disabled')) openModal(); });
    if (closeBtn) closeBtn.addEventListener('click', closeModal);
    if (overlay)  overlay.addEventListener('click', closeModal);

    if (doneBtn) {
      doneBtn.addEventListener('click', function () {
        if (doneBtn.hasAttribute('disabled')) return;
        form.requestSubmit();
      });
    }
