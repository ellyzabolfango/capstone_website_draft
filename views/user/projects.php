...
// list proposals by same location + type
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .upload-wrap { position:relative; display:inline-block; } .upload-wrap input[type=file]{ display:none; }
    .upload-btn { display:inline-block; padding:8px 16px; font-weight:700; font-size:12px; border-radius:999px; background:#e7e7e7; color:#1f1f1f; border:1px solid #cfcfcf; cursor:pointer; user-select:none; box-shadow:0 1px 0 rgba(0,0,0,0.15); letter-spacing:.5px; }
    .upload-btn:hover { filter:brightness(.96); } .upload-btn:active{ transform:translateY(1px); }
...
<body>
  ...
  <div class="main">
    ...
    <div class="fund-remaining">
      <div class="fund-badge"><span class="label">FUND REMAINING:</span><span class="amount"><?= htmlspecialchars(peso($fundRem), ENT_QUOTES) ?></span></div>
    </div>
    ...
    <div class="content-box">
      ...
      <?php if (!$rows): ?>
        <div style="opacity:.9; margin-top:14px;">No <?= strtolower($TYPE) ?>s found for <?= htmlspecialchars($userLoc ?: 'your location', ENT_QUOTES) ?>.</div>
      <?php else: ?>
