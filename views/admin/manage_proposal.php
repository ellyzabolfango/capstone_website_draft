<?php
// /views/manage_proposals.php — Admin only
session_start();
...
if (!empty($proposals)) {
  if ($id) {
    foreach ($proposals as $p) { if ($p['id'] === $id) { $selected = $p; break; } }
  }
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .detail {
      ...
      display: flex; flex-direction: column; gap: 12px; position: relative; color: #000;
    }
        ...
        <?php if ($selected): ?>
          ...
          <p><label>FILE ATTACHMENT:</label>
            <?php if ($selected['attachment']): ?>
              ...
              <a class="file-box" href="<?= htmlspecialchars($selected['attachment'], ENT_QUOTES) ?>" target="_blank" rel="noopener">📄 <?= htmlspecialchars($label, ENT_QUOTES) ?></a>
            <?php else: ?>
              <i>No file</i>
            <?php endif; ?>
          </p>
